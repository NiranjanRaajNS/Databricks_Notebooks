# Table Mapping: contactaddresstype → contact_type

## Overview
- **Legacy Database**: synergy_master
- **Legacy Schema**: enum
- **Legacy Table**: contactaddresstype
- **New Database**: smac_master_migration
- **New Schema**: crewing
- **New Table**: contact_type
- **Source Script**: `04-migration-scripts/master/contact_type_migration.sql`

- **Legacy Path**: `synergy_master.enum.contactaddresstype`
- **New Path**: `smac_master_migration.crewing.contact_type`
...
- Script performs `TRUNCATE TABLE crewing.contact_type` before insert (full table reload).
...
| 2 | contact_type_name | - | code | - | LEFT(UPPER(REPLACE(LEFT(TRIM(s.contact_type_name), 15), ' ', '_')), 50) AS code | LEFT(UPPER(REPLACE(LEFT(TRIM(s.contact_type_name), 15), ' ', '_')), 50) |
| 3 | contact_type_name | - | name | - | LEFT(COALESCE(s.contact_type_name, 'UNKNOWN'), 255) AS name | LEFT(COALESCE(s.contact_type_name, 'UNKNOWN'), 255) |
...
- `04-migration-scripts/master/contact_type_migration.sql`

## Validation

- Run `05-validation/master/contact_type_validation.sql` if available
- Run `06-rollback/master/contact_type_rollback.sql` if rollback is required
