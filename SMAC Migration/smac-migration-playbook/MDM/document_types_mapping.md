...
- Migrates document_types preserving identifier UUID as id. Master table with no dependencies.
...
|---|---------------|-------------|------------|----------|----------------|-------|
| 1 | id | - | id | - | migration.resolve_target_id() | migration.resolve_target_id( 'synergy_master'::VARCHAR(100), 'document'::VARCHAR(100), 'document_types'::VARCHAR(100), legacy_data.id::text, current_database()::text::VARCHAR(10... |
...
| 9 | name | - | level | - | (ROW_NUMBER() OVER (ORDER BY TRIM(legacy_data.name)) - 1)::numeric AS level | (ROW_NUMBER() OVER (ORDER BY TRIM(legacy_data.name)) - 1)::numeric |
