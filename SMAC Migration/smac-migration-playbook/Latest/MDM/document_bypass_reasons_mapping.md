# Table Mapping: document_bypass_reasons → document_bypass_reasons
...
- **New Schema**: document
- **New Table**: document_bypass_reasons
- **Source Script**: `04-migration-scripts/master/document_bypass_reasons_migration.sql`

- **Legacy Path**: `synergy_manning.public.document_bypass_reasons`
- **New Path**: `smac_master_migration.document.document_bypass_reasons`

## Business Key

- **Business Key**: `code`
- **Source (orchestration)**: Document Bypass Reasons (`document_bypass_reasons` → `document_bypass_reasons`)
...
- Source has only `id`, `uuid`, `name` — timestamps and `deleted_at` not in SAC
- `DISTINCT ON (id)` deduplication in dblink query


## Special Considerations

- Script performs `TRUNCATE TABLE document.document_bypass_reasons` before insert (full table reload).

## Column Mapping

| # | Legacy Column | Legacy Type | New Column | New Type | Transformation | Notes |
|---|---------------|-------------|------------|----------|----------------|-------|
| 1 | `id`, `uuid` | bigint, uuid | `id` | uuid | `migration.resolve_target_id()` — source_id = `id::text`; `p_target_id = uuid` | UUID preserved |
...
| 15 | — | — | `status` | integer | Hardcoded `0` (Active) | No `deleted_at` in SAC |
...
- `04-migration-scripts/master/document_bypass_reasons_migration.sql`

## Validation

- Run `05-validation/master/document_bypass_reasons_validation.sql` if available
- Run `06-rollback/master/document_bypass_reasons_rollback.sql` if rollback is required
