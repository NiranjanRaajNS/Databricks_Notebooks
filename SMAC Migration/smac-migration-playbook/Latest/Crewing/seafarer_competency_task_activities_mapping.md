...
- `seafarer_task_id` mapped via `migration.table_mappings` (`seafarer_competency_tasks`); unmapped rows excluded
- `data` (jsonb) transformed from legacy array to single JSON object; falls back to jsonb-to-text
- Uses `migration.build_audit_info()` with legacy `seafarer_task_id` in `notes`
- Requires `seafarer_competency_tasks` migrated first
...
| 3 | `data` | jsonb | `data` | text | LATERAL transform: array → latest element as structured JSON; else jsonb cast to text | Same pattern as subtask activities |
| 4 | `status` | text | `status` | text | `TRIM(COALESCE(status, ''))` | NOT NULL; empty string default |
| 5 | — | — | `tenant_id` | uuid | `:'DEFAULT_TENANT_ID'::uuid` from `constants.sql` | Standard tenant default |
| 6 | `created_at` | timestamp(6) | `created_at` | timestamp | `COALESCE(created_at, NOW())` | Direct copy with fallback |
| 7 | `updated_at` | timestamp(6) | `updated_at` | timestamp | `COALESCE(updated_at, NOW())` | Direct copy with fallback |
| 8 | — | — | `archived_at` | timestamp | `NULL` | No SAC equivalent |
| 9 | `deleted_at` | timestamp(6) | `deleted_at` | timestamp | Direct copy | Soft-delete preserved |
| 10 | `created_by_id`, `deleted_by_id`, `updated_by_id`, `created_by_name`, `updated_by_name` | varchar | `audit_info` | jsonb | `migration.build_audit_info()` — legacy `seafarer_task_id` in `notes` | Standardized SMAC audit structure |
| 11 | — | — | `name` | text | `NULL` | New optional SMAC field |
