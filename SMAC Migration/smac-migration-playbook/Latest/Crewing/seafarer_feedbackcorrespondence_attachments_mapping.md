...
- Filter: SAC `entity_type = 'FeedbackCorrespondence'` AND `entity_uuid IS NOT NULL` only
...
- `seafarer_id`: `seafarer_uuid_mapping` then `seafarer_id_mapping` fallback; nil UUID if unmapped
- `reference_entity` hardcoded `'seafarer_feedbacks'`; `reference_id` = join to `public.seafarer_feedbacks` on `entity_uuid`
- `file_content_type` split on `/` into `file_type` and `file_sub_type`
- `status`: `deleted_at IS NOT NULL` → `'DELETED'`, else `'ACTIVE'`
- Requires `seafarers` and `seafarer_feedbacks` migrated first
...
| 2 | `seafarer_uuid`, `seafarer_id` | character varying, bigint | `seafarer_id` | uuid | `COALESCE(seafarer_uuid_mapping.target_id, seafarer_id_mapping.new_id, nil UUID)` | Lookup: `public.seafarers` then `table_mappings` |
...
| 12 | `entity_uuid` | uuid | `reference_id` | uuid | Join `public.seafarer_feedbacks.id` on `entity_uuid` | Links attachment to parent feedback record |
...
**SAC columns not migrated:** `entity_type`, `entity_id` (used for filtering); `id` (bigint) used as `resolve_target_id` source_id only.
