...
- Migrates only attachments where entity_type = 'FeedbackComment' AND entity_id IS NULL
- Uses integer values for status, workflow_status, and defined_by (see constants.sql)
- Migrates seafarer_attachments table where entity_type = 'FeedbackComment' AND entity_uuid IS NULL. Preserves legacy uuid UUID when available. Maps seafarer_id via seafarers table. Sets reference_entity to 'seafarer_feedbacks'. Sets reference_id to NULL (entity_uuid is NULL in source). Splits file_content_type into file_type (before '/') and file_sub_type (after '/'). Sets default values for version_number (1) and status (ACTIVE/DELETED based on deleted_at). Requires seafarers and seafarer_feedbacks tables to be migrated first.
...
| 12 | - | - | reference_id | - | NULL | NULL::uuid |
