...
- **New Schema**: public
- **New Table**: roles
- **Source Script**: `04-migration-scripts/idp/seafarer/roles_migration.sql`

- **Legacy Path**: `IdentityAdmin_prod.public."Roles"`
...
- This migration is for seafarer role information from IdentityAdmin_prod database.
- Adjust based on actual source schema discovered via inspect_roles_schema.sql
...
- Script performs `TRUNCATE TABLE public.roles` before insert (full table reload).

## Column Mapping

| # | Legacy Column | Legacy Type | New Column | New Type | Transformation | Notes |
|---|---------------|-------------|------------|----------|----------------|-------|
| 1 | id_text | - | id | - | CASE WHEN legacy_data.id_text IS NOT NULL AND legacy_data.id_text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN legacy_data.id_text::uuid ELSE gen_ran... | CASE WHEN legacy_data.id_text IS NOT NULL AND legacy_data.id_text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN legacy_data.id_text::uuid ELSE gen_ran... |
...
| 18 | id_text | - | audit_info | - | migration.build_audit_info() | migration.build_audit_info( NULL::varchar, NULL::varchar, NULL::varchar, NULL::varchar, NULL::varchar, NULL::timestamp, NULL::varchar, NULL::text, NULL::varchar, NULL::text ) ||... |
...
- `04-migration-scripts/idp/seafarer/roles_migration.sql`
